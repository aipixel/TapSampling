...
from prismatic.extern.hf.modeling_prismatic import OpenVLAForActionPrediction, OpenVLAForActionPrediction_SP
...
def get_vla_sp(cfg: Any) -> torch.nn.Module:
    """
    Load and initialize the VLA model from checkpoint.
    ...
    # will be used as is, with its original modeling logic
    if not model_is_on_hf_hub(cfg.pretrained_checkpoint):
        print("Model is not on HF Hub, loading from local checkpoint:", cfg.pretrained_checkpoint)

        # Register OpenVLA model to HF Auto Classes (not needed if the model is on HF Hub)
        AutoConfig.register("openvla", OpenVLAConfig)
        AutoImageProcessor.register(OpenVLAConfig, PrismaticImageProcessor)
        AutoProcessor.register(OpenVLAConfig, PrismaticProcessor)
        AutoModelForVision2Seq.register(OpenVLAConfig, OpenVLAForActionPrediction_SP)
...
def get_action_head_SP(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead]:
    """
    Get action head for continuous value prediction.
    ...
    # Initialize appropriate action head based on configuration
    if cfg.use_l1_regression:
        action_head = L1RegressionActionHead_with_query_actions(
            input_dim=llm_dim, 
            hidden_dim=llm_dim, 
            action_dim=ACTION_DIM,
            use_pro_version=cfg.use_pro_version,
