...
LOCAL_POLICY_CONF_ROOT = TAPSAMPLING_ROOT / "policy_conf"
PRETRAINED_MODELS_ROOT = TAPSAMPLING_ROOT / "pretrained_models"
LOCAL_OPENVLA_CONFIG_JSON = PRETRAINED_MODELS_ROOT / "configs" / "config.json"
...
from prismatic.models.action_heads import L1RegressionActionHead, L1RegressionActionHead_with_query_actions
...
from prismatic.util.data_utils import PaddedCollatorForActionPrediction, PaddedCollatorForActionPrediction_SP
...
from prismatic.vla.datasets import RLDSDataset, RLDSBatchTransform, CalvinBatchTransform
...
import yaml
def _convert_paths_and_primitives(obj: Any):
    if isinstance(obj, dict):
        return {k: _convert_paths_and_primitives(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        converted = [_convert_paths_and_primitives(v) for v in obj]
    ...
    if isinstance(obj, (str, int, float, bool, type(None))):
        return obj
    if is_dataclass(obj):
        return _convert_paths_and_primitives(asdict(obj))
    if hasattr(obj, "__dict__"):
        return _convert_paths_and_primitives(vars(obj))
    return str(obj)

def dataclass_to_serializable_dict(cfg: Any) -> dict:
    if is_dataclass(cfg):
        base = asdict(cfg)
        ...
        raise ValueError(f"Unsupported cfg type: {type(cfg)}")
    return _convert_paths_and_primitives(base)

def save_dataclass_to_yaml(cfg: Any, out_path: str):
    container = dataclass_to_serializable_dict(cfg)
    with open(out_path, "w", encoding="utf-8") as f:
        yaml.safe_dump(container, f, sort_keys=False, allow_unicode=True)
...
class FinetuneConfig:
    ...
    policy_config_name: str = "Success_Percent_Model"  # Local Hydra config name under tapsampling/policy_conf

    # Dataset
    dataset_name: str = "calvin_abc"    # Name of fine-tuning dataset (e.g., `aloha_scoop_x_into_bowl`)
    ...
    use_realtive_percent: bool = True
...
def run_forward_pass(
    ...
    target_success_percent = batch["success_percent"].to(device_id).to(torch.bfloat16)
    # print("target_success_percent", target_success_percent.shape)
    ...
    predicted_success_percent = action_head.module.predict_action(
        multi_layer_hidden_states,
        proprio=batch["proprio"] if use_proprio else None,
        proprio_projector=proprio_projector if use_proprio else None,
        phase=cfg.phase,
        query_actions = batch["query_actions"].to(device_id).to(torch.bfloat16)
        )
    
    # print("predicted_success_percent", predicted_success_percent.shape)
    # print("target_success_percent", target_success_percent.shape)

    predicted_success_percent = predicted_success_percent.squeeze(-1).mean(-1)
    if len(target_success_percent.shape) == 1:
        target_success_percent = target_success_percent.unsqueeze(0)

    assert len(target_success_percent.shape) ==2, print("target_success_percent", target_success_percent.shape, target_success_percent)
    target_success_percent = target_success_percent[:, -1]

    # print("predicted_success_percent", predicted_success_percent)
    # print("target_success_percent", target_success_percent)
    loss = torch.nn.L1Loss()(predicted_success_percent, target_success_percent)

    # target_success_percent
    # loss = torch.nn.L1Loss()(predicted_actions, ground_truth_actions)
    tp_num =  (torch.sign(predicted_success_percent) == torch.sign(target_success_percent)).sum().item()
    metrics.update(
        {
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        if cfg.use_minivlm:
            config = AutoConfig.from_pretrained(str(LOCAL_OPENVLA_CONFIG_JSON))
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    (next-token prediction vs. L1 regression), FiLM. Also allows for additional model inputs,
    ...
    cfg_save = deepcopy(cfg)
    print("save to ", f"{run_dir}/config_resolved.yaml")
    save_dataclass_to_yaml(cfg_save, f"{run_dir}/config_resolved.yaml")
    ...
    L1RegressionActionHead_with_query_actions,
    "action_head",
    cfg,
    device_id,
    {
    ...
    batch_transform = CalvinBatchTransform(
        action_tokenizer,
        processor.tokenizer,
        ...
        use_realtive_percent = cfg.use_realtive_percent
        )
    ...
    with initialize_config_dir(config_dir=str(LOCAL_POLICY_CONF_ROOT), job_name="job_name"):
        cfgdata = compose(config_name=cfg.policy_config_name)
    dataset_root = Path(os.environ.get("CALVIN_DATASET_ROOT"))
    cfgdata.root_data_dir = dataset_root.as_posix()
    train_dataset = hydra.utils.instantiate(cfgdata.datamodule.datasets)["lang_dataset"]
    train_dataset.dataset_transform = batch_transform

    # print("len dataset", len(train_dataset))
    # print(train_dataset[99])

    collator = PaddedCollatorForActionPrediction_SP(
    ...
    dataloader = DataLoader(
        ...
        collate_fn=collator,
        shuffle = True,
        num_workers=16,  # Important: Set to 0 if using RLDS, which uses its own parallelism
        drop_last = True,
    )
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False, dynamic_ncols=True) as progress:
        ...
        for epoch_idx in range(50):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                    ...
                    epoch_loss += metrics['loss_value']
                    avg_loss = epoch_loss / (loss_avg_step + 1)
                    loss_avg_step += 1
                    ...
                    epoch_tp += metrics["tp_num"]
                    epoch_all += cfg.batch_size
                    avg_acc = epoch_tp / epoch_all
                    progress.set_postfix(
                        {
                            "epoch loss": f"{avg_loss:.4f}",
                            "epoch acc": f"{100*avg_acc:.1f}",
                        },
                        refresh=False,  # refresh=False is slightly cheaper; progress.update() will redraw
                    )
                    progress.update()
