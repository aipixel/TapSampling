...
class CalvinBatchTransform:
    ...
    use_realtive_percent: bool = False
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ['rgb_static', 'rgb_gripper', 'robot_obs', 
        #  'scene_obs', 'actions', 'language', 
        #  'language_text', 'rel_actions', 'success_percent']

        # print(rlds_batch["rgb_static"].shape)
        img = Image.fromarray(rlds_batch["rgb_static"][0]).resize((224,224), resample=Image.BILINEAR)
        pixel_values = self.image_transform(img)
        # print("pixel_values", type(pixel_values))
        # print(pixel_values.shape)

        lang = rlds_batch["language_text"].lower()
        actions = torch.from_numpy(rlds_batch["rel_actions"])
        ...
        if self.use_minivlm:
            ...
            future_actions_string = self.action_tokenizer(actions, self.use_minivlm)

            action_chunk_string = future_actions_string
            ...
            # print("before prompt", prompt_builder.get_prompt().encode("unicode_escape").decode("utf-8"))
            ...
            # print(f"current action length {action_chunk_len}, will be processed to {NUM_TOKENS}")
        ...
        labels[: -(action_chunk_len)] = IGNORE_INDEX
        if not self.predict_stop_token:
            labels[-1] = IGNORE_INDEX

        return_dict = dict(pixel_values=pixel_values, 
                           input_ids=input_ids, 
                           labels=labels, 
                           dataset_name='calvin', 
        ...
        if self.use_wrist_image:
            img_wrist = Image.fromarray(rlds_batch["rgb_gripper"][0]).resize((224,224), resample=Image.BILINEAR)
            all_wrist_pixels = [ self.image_transform(img_wrist) ] 
            return_dict["pixel_values_wrist"] = torch.cat(all_wrist_pixels, dim=0)
        if self.use_proprio and "proprio" in rlds_batch["observation"]:
            proprio = rlds_batch["observation"]["proprio"]
            return_dict["proprio"] = proprio

        # print("rlds_batch[success_percent]", rlds_batch["success_percent"].shape, rlds_batch["success_percent"])
        if self.use_realtive_percent:
            return_dict["success_percent"] = rlds_batch["success_percent"] - rlds_batch["success_percent"][0]
        else:
            return_dict["success_percent"] = rlds_batch["success_percent"]
        return_dict["success_percent"] = torch.from_numpy(return_dict["success_percent"])
        ...
        #         print(v.shape)
        # aaa
        # print("sp", return_dict["success_percent"])
