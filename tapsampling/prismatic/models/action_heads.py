...
class L1RegressionActionHead_with_query_actions(nn.Module):
    """输入query_actions, 预测成功率"""
    def __init__(
        self,
        input_dim=4096,
        hidden_dim=4096,
        action_dim=6,               # not output! this is query action dim!     eepose + eeorient
        num_task_tokens=512,
        ...
        self.model = MLPResNet(                 # (b, 10, 7) -> (b, 10, 4096) -> (b, 10, 1)
            num_blocks=24, 
            input_dim=input_dim, 
        ...
        self.query_action_emb = nn.Linear(action_dim, hidden_dim)
    ...
    def predict_action(
        ...
        # print("dtype", actions_hidden_states.dtype, query_actions.dtype)
        # aa
        ...
        # (b 10 6) -> (b, 10, 4096)
        rearranged_actions_hidden_states = self.query_action_emb(query_actions)
        # print("rearranged_actions_hidden_states (query actions emb)", rearranged_actions_hidden_states.shape)
        # assert cond_actions_hidden_states.shape[1]
        ...
        # print("rearranged_actions_hidden_states", rearranged_actions_hidden_states.shape)
        # print("actions_hidden_states", actions_hidden_states.shape)
        # print("task_hidden_states", task_hidden_states.shape)
