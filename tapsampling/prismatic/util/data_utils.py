...
@dataclass
class PaddedCollatorForActionPrediction_SP:
    model_max_length: int
    pad_token_id: int
    padding_side: str = "right"
    pixel_values_dtype: torch.dtype = torch.float32
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        # Stack all actions
        actions = [torch.from_numpy(np.copy(instance["query_actions"])) for instance in instances]
        actions = torch.stack(actions)
        ...
        success_percent = [instance["success_percent"] for instance in instances]
        success_percent = torch.Tensor(np.squeeze(np.stack(success_percent)))
