...
def get_mode_beso_and_env(train_folder, dataset_path, checkpoint, env=None, lang_embeddings=None, device_id=0, eval_cfg_overwrite={},cfg=None,
                          datamodule_for_mode = None):
    ...
    ###
    datamodule_for_mode.root_data_dir = dataset_path
    data_module22 = hydra.utils.instantiate(datamodule_for_mode, num_workers=0)
    data_module22.prepare_data()
    data_module22.setup()
    dataloader22 = data_module22.val_dataloader()
    dataset22 = dataloader22["lang"].dataset

    # print(dataset22.transforms)
    # aaa
    ...
    if env is None:
        rollout_cfg = OmegaConf.load(Path(__file__).parents[1] / "policy_conf/callbacks/rollout/formode.yaml")
        env = hydra.utils.instantiate(rollout_cfg.env_cfg, dataset, device, show_gui=False,
                                      dataset_loader_for_mode=dataset22)
