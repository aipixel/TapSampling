...
class VPP_Skeleton(pl.LightningModule):
    """
    ...
    def __init__(
            ...
            gcdenoiser_dual_adapter = True,
            skeleton_detach = False
    ):
        super(VPP_Skeleton, self).__init__()
        ...
        self.model = GCDenoiser(action_dim = action_dim,
                                ...
                                dual_former = dual_former,
                                skeleton_dim = skeleton_dim,
                                bidirection = bidirection,
                                cross_after_blocks = cross_after_blocks,
                                gcdenoiser_decoder_layer = gcdenoiser_decoder_layer, 
    ...
    def training_step(self, dataset_batch: Dict[str, Dict],) -> torch.Tensor:  # type: ignore
        ...
        skeleton_loss = torch.tensor(0.0).to(self.device)
        ...
        if not self.dual_former:
            ...
            action_loss += act_loss["loss_action"]
            skeleton_loss += act_loss["loss_skeleton"]

            total_loss += act_loss["loss_action"]
            total_loss += act_loss["loss_skeleton"]
        
            total_bs = dataset_batch["actions"].shape[0]

            self._log_training_metrics(action_loss, total_loss, total_bs)
            return total_loss, action_loss, skeleton_loss
    ...
    def extract_predictive_feature(self, dataset_batch):
        ...
        # print("2 perceptual_features after Video_Former", perceptual_features.shape)      # b, 224, 384
    ...
    def diffusion_loss(
        ...
        if not self.dual_former:
            ...
            loss, loss_skeleton, _ = self.model.loss(perceptual_emb, actions, latent_goal, noise, sigmas)
            return {'loss_action': loss, 'loss_skeleton': loss_skeleton}, sigmas, noise
    ...
    def denoise_actions(  # type: ignore
        ...
        x = torch.randn((len(latent_goal), self.act_window_size, self.action_dim + self.skeleton_dim), device=self.device) * self.sigma_max

        if return_noise:
            noise = x / self.sigma_max
    ...
    def step(self, obs, goal, with_video_skeleton = False,
             return_full_skeleton = False):
        """
        ...
        if not with_video_skeleton:
            if self.rollout_step_counter % self.multistep == 0:
                pred_action_seq = self.eval_forward(obs, goal)
                pred_action_seq = pred_action_seq[..., :self.action_dim]        # 后面是skeleton
            ...
            if self.rollout_step_counter % self.multistep == 0:
                ##### 新推理、更新skeleton
                pred_action_seq_origin = self.eval_forward(obs, goal)
                pred_action_seq = pred_action_seq_origin[..., :self.action_dim]        # 后面是skeleton
                pred_skeleton_seq = pred_action_seq_origin[..., self.action_dim:]
                self.pred_action_seq = pred_action_seq
                self.pred_skeleton_seq = pred_skeleton_seq
            ...
            if return_full_skeleton:
                current_skeleton = self.pred_skeleton_seq
            else:
                current_skeleton = self.pred_skeleton_seq[0, self.rollout_step_counter]
            if len(current_skeleton.shape) == 2:
                current_skeleton = einops.rearrange(current_skeleton, 'b d -> b 1 d')

            ## 因为是新chunk，所以更新obs，需要利用新的真实obs初始化，后续计算骨骼相对运动
            ## 如果没到新chunk，就继续上一步预测的骨骼进行计算
            new_chunk_new_obs = (self.rollout_step_counter==0)

            self.rollout_step_counter += 1
            if self.rollout_step_counter == self.multistep:
                self.rollout_step_counter = 0

            return current_action, current_skeleton, None, new_chunk_new_obs   #暂时没写预测的视频

    def reset_with_noise_and_latents(self):
        """
        Call this at the beginning of a new rollout when doing inference.
        """
        self.plan = None
        ...
        # self.pred_used_noise = None

    def eval_forward_return_noise_and_latents(self, obs, goal):
        """
        Method for doing inference with the model.
        """
        if 'lang_text' in goal:
        ...
        with torch.no_grad():
            input_rgb = torch.cat([rgb_static, rgb_gripper], dim=0)
            language = [language] + [language]
            perceptual_features, rand_latents = self.TVP_encoder.forward_return_latents(input_rgb, language, self.timestep,
                                                           self.extract_layer_idx, all_layer=self.use_all_layer,
                                                           step_time=1, max_length=self.max_length)
            rand_latents = rand_latents / self.TVP_encoder.pipeline.scheduler.init_noise_sigma
            
            noises["noise_svd"] = rand_latents
            # print("TVP noise:", rand_latents.shape, rand_latents.mean(), rand_latents.std())
            # TVP noise: torch.Size([2, 16, 4, 32, 32]) 
            # tensor(-3.6055, device='cuda:1', dtype=torch.float16) 
            # tensor(699.5000, device='cuda:1', dtype=torch.float16)
            #
            # TVP noise: torch.Size([2, 16, 4, 32, 32]) 
            # tensor(-0.0051, device='cuda:1', dtype=torch.float16) 
            # tensor(0.9990, device='cuda:1', dtype=torch.float16)
        ...
        act_seq, noise = self.denoise_actions(
            torch.zeros_like(latent_goal).to(latent_goal.device),
            perceptual_emb,
            latent_goal,
            inference=True,
        ...
        noises["noise_dp"] = noise.clone()

        return act_seq, noises

    def step_return_noise_and_latents(self, obs, goal, with_video_skeleton = False):
        """
        Do one step of inference with the model. THis method handles the action chunking case.
        Our model is trained to predict a sequence of actions.
        We only compute the sequence once every self.multistep steps.
        ...
        if not with_video_skeleton:
            if self.rollout_step_counter % self.multistep == 0:
                pred_action_seq, noises = self.eval_forward_return_noise_and_latents(obs, goal)
                pred_action_seq = pred_action_seq[..., :self.action_dim]        # 后面是skeleton
                self.pred_action_seq = pred_action_seq

                self.pred_used_noise = noises

                
                # for k, v in noises.items():
                #     print(k, v.shape, v.mean(), v.std())
                # noise_svd torch.Size([2, 16, 4, 32, 32]) 
                # tensor(-0.0051, device='cuda:1', dtype=torch.float16) 
                # tensor(0.9990, device='cuda:1', dtype=torch.float16)
                # noise_dp torch.Size([1, 10, 28]) 
                # tensor(0.0560, device='cuda:1') 
                # tensor(0.9640, device='cuda:1')
            ...
            if self.rollout_step_counter == self.multistep:
                self.rollout_step_counter = 0
                                                                                            # static + gripper
            current_svd_noise = self.pred_used_noise["noise_svd"][:, self.rollout_step_counter, ...] # 2, 4, 32, 32
            current_dp_noise = self.pred_used_noise["noise_dp"][:, self.rollout_step_counter, ...]   # 1, 28

            return current_action, current_svd_noise, current_dp_noise
    ...
    def reset_and_set_svd_noise(self, loaded_svd_noise):
        """
        Call this at the beginning of a new rollout when doing inference.
        """
        self.plan = None
        ...
        self.loaded_svd_noise = loaded_svd_noise

        # self.pred_used_noise = None

    def eval_forward_with_loaded_svd_noise(self, obs, goal):
        """
        Method for doing inference with the model.
        """
        if 'lang_text' in goal:
        ...
        if self.loaded_svd_noise is None:
            current_noise = None
        else:
            # N, 2, 4, 32, 32
            if self.loaded_svd_noise.shape[0] > 10:
                current_noise = deepcopy(self.loaded_svd_noise[:10, ...])         # 10, 2, 4, 32, 32
                self.loaded_svd_noise = self.loaded_svd_noise[10:, ...]
            else:
                self.loaded_svd_noise = None
                current_noise = None

        with torch.no_grad():
            input_rgb = torch.cat([rgb_static, rgb_gripper], dim=0)
            language = [language] + [language]
            perceptual_features, rand_latents = self.TVP_encoder.forward_given_noise(input_rgb, language, self.timestep,
                                                           self.extract_layer_idx, all_layer=self.use_all_layer,
                                                           step_time=1, max_length=self.max_length,
                                                           noise = current_noise
                                                           )
    ...
    def step_with_loaded_svd_noise(self, obs, goal, with_video_skeleton = False):

        if not with_video_skeleton:
            if self.rollout_step_counter % self.multistep == 0:
                pred_action_seq, noises = self.eval_forward_with_loaded_svd_noise(obs, goal)
                pred_action_seq = pred_action_seq[..., :self.action_dim]        # 后面是skeleton
                self.pred_action_seq = pred_action_seq
