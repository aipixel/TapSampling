...
class DiffusionTransformer(nn.Module):
    ...
    def forward_dec_only(self, context, actions, sigma):
        ...
        # print("actions", actions.shape)                 # b, 10, 7
        action_embed = self.action_emb(actions)
        # print("action_embed", action_embed.shape)       # b, 10, 384
    ...
    def concatenate_inputs(self, emb_t, goal_x, state_x, proprio_x, uncond=False):
        ...
        if self.goal_conditioned:                       # 这是文本的goal
            input_seq_components.insert(0, goal_x)

        if proprio_x is not None:
            #print("diffusion_decoder, 267, use proprio")
            input_seq_components.append(proprio_x)                      # VPP没有使用proprio！
        # else:
        #     print("diffusion_decoder, 270, not use proprio")
        #else:
        #    if not self.goal_conditioned:
        #        input_seq_components.append(self.drop(goal_x))

        input_seq = torch.cat(input_seq_components, dim=1)
        # print("diffusion_decoder, 276, input_seq.shape:", input_seq.shape)
        return input_seq
...
class Dual_DiffusionTransformer(nn.Module):
    """the full GPT score model, with a context size of block_size"""
    ...
    def __init__(
        ...
        self.decoder = Dual_TransformerFiLMDecoder(
            embed_dim=embed_dim,
            n_heads=n_heads,
            attn_pdrop=attn_pdrop,
            resid_pdrop=resid_pdrop,
        ...
        self.action_emb = nn.Linear(action_dim, embed_dim)
        self.skeleton_emb = nn.Linear(skeleton_dim, embed_dim)


        if linear_output:
            self.action_pred = nn.Linear(embed_dim, self.action_dim)
            self.skeleton_pred = nn.Linear(embed_dim, self.skeleton_dim)
        else:
            self.action_pred = nn.Sequential(
                nn.Linear(embed_dim, 100),
                nn.GELU(),
            ...
            self.skeleton_pred = nn.Sequential(
                nn.Linear(embed_dim, 100),
                nn.GELU(),
                nn.Linear(100, self.skeleton_dim)
            )
    ...
    def forward_dec_only(self, context, actions, sigma):
        ...
        assert actions.shape[-1] > 7, "Dual_Transformer need 'action + skeleton'"
        actions_actions, actions_skeleton = torch.split(actions, [7, actions.shape[-1] - 7], dim = -1)

        actions_embed = self.action_emb(actions_actions)
        action_x = self.drop(actions_embed)
        skeleton_embed = self.skeleton_emb(actions_skeleton)
        skeleton_x = self.drop(skeleton_embed)

        x, skeleton = self.decoder(action_x, skeleton_x, emb_t, context)
        pred_actions = self.action_pred(x)
        pred_skeleton = self.skeleton_pred(skeleton)
        return torch.cat([pred_actions, pred_skeleton], dim = -1)
...
class Dual_DiffusionTransformer_Dual_Encoder(nn.Module):
    """the full GPT score model, with a context size of block_size"""
    ...
    def __init__(
        ...
        print(f"obs dim: {obs_dim}, goal_dim: {goal_dim}, action_dim: {action_dim}, proprio_dim: {proprio_dim}")
        self.tok_emb = nn.Linear(obs_dim, embed_dim)
        self.tok_emb_vggt = nn.Linear(obs_dim, embed_dim)
        if use_mlp_goal:
            self.goal_emb = nn.Sequential(
                nn.Linear(goal_dim, embed_dim * 2),
                nn.GELU(),
        ...
        self.encoder_vggt = TransformerEncoder(
            embed_dim=embed_dim,
            n_heads=n_heads,
            attn_pdrop=attn_pdrop,
            resid_pdrop=resid_pdrop,
        ...
        self.decoder = Dual_TransformerFiLMDecoder_vggt(
            embed_dim=embed_dim,
            n_heads=n_heads,
            attn_pdrop=attn_pdrop,
            resid_pdrop=resid_pdrop,
    ...
    def forward(self, states, actions, goals, sigma, uncond: Optional[bool] = False):
        context, context_vggt = self.forward_enc_only(states, actions, goals, sigma, uncond)
        #print("dec only!")
        pred_actions = self.forward_dec_only(context, context_vggt, actions, sigma)
        return pred_actions

    def forward_enc_only(self, states, actions=None, goals=None, sigma=None, uncond: Optional[bool] = False):
        emb_t = self.process_sigma_embeddings(sigma) if not self.use_ada_conditioning else None
        goals = self.preprocess_goals(goals, states['state_images'].size(1), uncond)
        state_embed, proprio_embed, state_embed_vggt = self.process_state_embeddings(states)
        goal_embed = self.process_goal_embeddings(goals)
        ...
        context_vggt = self.encoder_vggt(state_embed_vggt)


        return context, context_vggt

    def forward_dec_only(self, context, context_vggt, actions, sigma):
        #print("process_sigma_embeddings only!")
        emb_t = self.process_sigma_embeddings(sigma)
        ...
        x, skeleton = self.decoder(action_x, skeleton_x, emb_t, context, context_vggt)
        pred_actions = self.action_pred(x)
        pred_skeleton = self.skeleton_pred(skeleton)
        return torch.cat([pred_actions, pred_skeleton], dim = -1)
    ...
    def process_state_embeddings(self, states):
        states_global = self.tok_emb(states['state_images'])
        states_vggt = self.tok_emb_vggt(states['vggt_features'])
        if 'state_obs' in states:
            proprio_embed = self.proprio_emb(states['state_obs'])
        else:
            proprio_embed = None
        return states_global, proprio_embed, states_vggt
