...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y=None) -> torch.Tensor:
        ...
        # print("k:", k.shape)
        # print("v:", v.shape)
        # print("attn_mask:", self.attn_mask.shape, self.attn_mask)
        # aa
...
class Video_Former_3D(nn.Module):
    ...
    def forward_all_layers(self, 
        ...
        if self.use_temporal:
            for (attn, Temp_attn, ffw), norm_layer in zip(self.layers, self.norm):
                ...
                if self.return_all_layers:
                    im_x = x.reshape(batch_size, -1 ,x.shape[1],x.shape[2])
                    im_x = rearrange(im_x, 'b T q d -> b (T q) d')
                    im_x = norm_layer(im_x)
                    all_features.append(im_x)
    ...
    def forward(self, x_f: torch.Tensor, mask: torch.BoolTensor = None, extra : torch.Tensor = None):
        ...
        if self.return_all_layers:
            return self.forward_all_layers(x_f,
                                           mask,
                                           extra)
...
##
# from .transformers.moe import ModelArgs, MoE
class Video_Former_3D_MoE(nn.Module):
    """Perceiver Resampler with multi-head attention layer"""
    ...
    def __init__(
        ...
        for k, v in moe_config.items():
            print(k, v)
            setattr(self.moe_args, k, v)
        print("Moe Config:", self.moe_args)


        if self.moe_args.cond_dim > 0:
            self.moe_cond_embed = nn.Linear(512, self.moe_args.cond_dim)
        else:
            self.moe_cond_embed = None
    ...
    def forward(self, x_f: torch.Tensor, mask: torch.BoolTensor = None, extra : torch.Tensor = None,
        ...
        if moe_cond is not None:
            moe_cond = self.moe_cond_embed(moe_cond)
...
class Dynamic_Spatial_Fusion_Model(nn.Module):
    """Perceiver Resampler with multi-head attention layer"""
