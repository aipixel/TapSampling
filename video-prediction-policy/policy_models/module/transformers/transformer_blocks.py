...
class Block_multicond(nn.Module):
    ...
    def __init__(
        ...
        if self.use_cross_attention:
            ...
            self.cross_att_vggt = Attention(n_embd, n_heads, attn_pdrop, resid_pdrop, block_size, causal, bias, use_rot_embed, rotary_xpos)
            self.ln3 = nn.LayerNorm(n_embd)

            self.cross_att_skeleton = Attention(n_embd, n_heads, attn_pdrop, resid_pdrop, block_size, causal, bias, use_rot_embed, rotary_xpos)
            self.ln3 = nn.LayerNorm(n_embd)
        self.ln_2 = LayerNorm(n_embd, bias=bias)
        self.mlp = MLP(n_embd, bias, mlp_pdrop)
...
class ConditionedBlock_MultiCond(Block_multicond):
    """
...
class TransformerFiLMDecoder(nn.Module):
    ...
    def forward(self, x, c, cond=None, custom_attn_mask=None):
        for idx, layer in enumerate(self.blocks):
            ...
            # print("FilmDecoder", idx, x.shape, c.shape, cond.shape, custom_attn_mask)
        x = self.ln(x)
        return x


from .action_head import MLPResNetBlock_Pro_3Cond
class Dual_TransformerFiLMDecoder_vggt(nn.Module):
    def __init__(
            self, 
            embed_dim: int, 
            n_heads: int, 
        ...
        self.gcdenoiser_dual_adapter = gcdenoiser_dual_adapter
        self.skeleton_detach = skeleton_detach

        if self.gcdenoiser_dual_adapter:
            self.adapter_s2a = nn.ModuleList([
                nn.Sequential(
            ...
            self.adapter_a2s = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(embed_dim, embed_dim),
                    nn.GELU(),
                    nn.Linear(embed_dim, embed_dim)
        ...
        self.cross_after_blocks = cross_after_blocks
        ...
        if use_noise_encoder:
            ...
            self.blocks_action = nn.Sequential(
                *[MLPResNetBlock_Pro_3Cond(dim = embed_dim,
                                           num_heads = n_heads)
                  for _ in range(n_layers)]
            )

            self.blocks_skeleton = nn.Sequential(
                *[MLPResNetBlock_Pro_3Cond(dim = embed_dim,
                                           num_heads = n_heads)
                  for _ in range(n_layers)]
            )
    ...
    def forward(self, x, skeleton, c, cond=None, cond_vggt=None, custom_attn_mask=None):
        
        # self.bidirection
        # self.cross_after_blocks       # 只有单向时可行

        # print("transformer_blocks.py -- 720, ?")
        if self.gcdenoiser_dual_adapter:
            assert self.bidirection, "onedirection is not implemented"
            for layer_action, layer_skeleton, layer_a2s, layer_s2a in zip(self.blocks_action,  self.blocks_skeleton, self.adapter_a2s, self.adapter_s2a):
                x_prev = x + layer_a2s(x)
                skeleton_prev = skeleton + layer_s2a(skeleton)
                x = layer_action(x, 
                                 h_a = cond, 
                                 h_t = cond_vggt,
                                 h_addition = skeleton_prev,
                                 c = c)
                skeleton = layer_skeleton(skeleton, 
                                        h_a = cond, 
                                        h_t = cond_vggt,
                                        h_addition = x_prev,
                                        c = c)
                # x = layer_action(x, c, torch.cat([cond, skeleton_prev], dim = 1), custom_attn_mask=custom_attn_mask)
                # skeleton = layer_skeleton(skeleton, c, torch.cat([cond, x_prev], dim = 1), custom_attn_mask=custom_attn_mask)

                    
        x = self.ln(x)
        skeleton = self.ln_2(skeleton)
        return x, skeleton


class Dual_TransformerFiLMDecoder(nn.Module):
    def __init__(
            self, 
            embed_dim: int, 
            n_heads: int, 
    ...
    def forward(self, x, skeleton, c, cond=None, custom_attn_mask=None):
        
        # self.bidirection
        # self.cross_after_blocks       # 只有单向时可行

        if self.skeleton_detach:
            if self.gcdenoiser_dual_adapter:
                # print("skeleton_detach, gcdenoiser_dual_adapter")
                assert self.bidirection, "onedirection is not implemented"
                for layer_action, layer_skeleton, layer_a2s, layer_s2a in zip(self.blocks_action,  self.blocks_skeleton, self.adapter_a2s, self.adapter_s2a):
                    x_prev = x.detach() + layer_a2s(x.detach())
                    skeleton_prev = skeleton + layer_s2a(skeleton)
                    x = layer_action(x, c, torch.cat([cond, skeleton_prev], dim = 1), custom_attn_mask=custom_attn_mask)
                    skeleton = layer_skeleton(skeleton, c, torch.cat([cond, x_prev], dim = 1), custom_attn_mask=custom_attn_mask)
                ...
                raise NotImplementedError("")
                for layer_action, layer_skeleton in zip(self.blocks_action,  self.blocks_skeleton):
                    if self.bidirection:
                        x_prev = x
                        ...
                        if self.cross_after_blocks:
                            skeleton = layer_skeleton(skeleton, c, cond, custom_attn_mask=custom_attn_mask)
                            x = layer_action(x, c, torch.cat([cond, skeleton], dim = 1), custom_attn_mask=custom_attn_mask)
                        else:
                            x = layer_action(x, c, torch.cat([cond, skeleton], dim = 1), custom_attn_mask=custom_attn_mask)
                            skeleton = layer_skeleton(skeleton, c, cond, custom_attn_mask=custom_attn_mask)
