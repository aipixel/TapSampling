...
class VPP_Policy(pl.LightningModule):
    ...
    def denoise_actions(  # type: ignore
        ...
        assert find in ['best', 'worst', 'none']
        ...
        # print("latent_plan", latent_plan.shape)
        # print("perceptual_emb['state_images']", perceptual_emb['state_images'].shape)

        if sample_num > 1 and latent_goal.shape[0] != sample_num:
            latent_goal = latent_goal.repeat(sample_num, 1, 1)
        if sample_num > 1 and latent_plan.shape[0] != sample_num:
            latent_plan = latent_plan.repeat(sample_num, 1, 1)
        if sample_num > 1 and perceptual_emb['state_images'].shape[0] != sample_num:
            perceptual_emb['state_images'] = perceptual_emb['state_images'].repeat(sample_num, 1, 1)

        if batch_num > 1:
            raise NotImplementedError("")
        # latent_goal = latent_goal.repeat(batch_num, 1, 1)
        # latent_plan = latent_plan.repeat(batch_num, 1, 1)
        # perceptual_emb['state_images'] = perceptual_emb['state_images'].repeat(batch_num, 1, 1)
        ...
        actions, minmax = self.sample_loop(sigmas, x, input_state, latent_goal, latent_plan, self.sampler_type, extra_args,
                                    batch_num = 1,
                                    find = find,)
    ...
    def sample_loop(
        ...
        # print("sampler_type", sampler_type)   tensor([80.0000, 49.1627, 30.2121, 18.5664, 11.4096,  7.0116,  4.3089,  2.6479, 1.6273,  1.0000,  0.0000], device='cuda:1')
        # print("sigmas", sigmas)
        ...
        # 2nd order solver
            ...
            x_0, x_mid = sample_ddim(self.model, state, x_t, goal, sigmas, scaler=scaler, disable=True,
                              difsteps = difsteps)   # 返回一个5步扩散后的action
            # x_t_inversion = basic_ddim_inversion(self.model, state, x_mid, goal, sigmas, disable=True, step = difsteps)
            minmax = 0
            if batch_num > 1:

                # print("x_t", x_t.shape, x_t.max(), x_t.min(), x_t.std())
                # print("x_t_inversion", x_t_inversion.shape, 
                #     x_t_inversion.max(), 
                #     x_t_inversion.min(), 
                #     x_t_inversion.std())

                x_flat = (x_t/sigmas[0]).view(batch_num, -1)                    # b, 70
                y_flat = (x_t_inversion/sigmas[0]).view(batch_num, -1)          # b, 70


                dis = (x_flat - y_flat).abs().mean(dim=1)

                if self.infer_metric == 'l1':
                    dis = (x_flat - y_flat).abs().mean(dim=1)
                    # print("dis", dis)
                    # print(dis.max() - dis.min())
                    minmax = (dis.max() - dis.min()).detach().cpu().item()
                    if find == 'best':
                        idx = torch.argmin(dis)
                    elif find == 'worst':
                        idx = torch.argmax(dis)

                if self.infer_metric == 'cos':
                    cos_sim = F.cosine_similarity(x_flat, y_flat, dim=1)
                    # print("cos_sim", cos_sim)
                    minmax = (cos_sim.max() - cos_sim.min()).detach().cpu().item()
                    if find == 'best':
                        idx = torch.argmax(cos_sim)
                    elif find == 'worst':
                        idx = torch.argmin(cos_sim)
    ...
    def eval_forward(self, obs, goal,
        ...
        # print("aaa", rgb_static.shape,rgb_gripper.shape, batch)
        ...
        act_seq, minmax = self.denoise_actions(
            torch.zeros_like(latent_goal).to(latent_goal.device),
            perceptual_emb,
            latent_goal,
            inference=True,
    ...
    def step(self, obs, goal,
        ...
        if self.rollout_step_counter % self.multistep == 0:
            pred_action_seq, minmax = self.eval_forward(obs, goal,
                                                batch_num = batch_num,
                                                find = find,
                                                
                                                sample_num = sample_num)    # 返回N个采样的运动
        ...
        return current_action, self.minmax
