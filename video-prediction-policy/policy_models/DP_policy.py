...
from policy_models.module.dino import Dino
...
class DP_Policy(pl.LightningModule):
    """
    DP: DinoV2 + Q-Fromer + DiT
    """
    ...
    def __init__(
        ...
        if use_Former=='3d':
            ...
            self.Video_Former = nn.Linear(1024,latent_dim)
    ...
    def extract_predictive_feature(self, dataset_batch):
        ...
        with torch.no_grad():
            input_rgb = torch.cat([rgb_static, rgb_gripper], dim=0).squeeze(1)  # b*2, 3, 518, 518   # rgb+gripper
            language = language + language
            perceptual_features = self.TVP_encoder(input_rgb).unsqueeze(1)       # b*2, 1, 1369, 1024

        perceptual_features, gripper_feature = torch.split(perceptual_features, [batch, batch], dim=0)
        perceptual_features = torch.cat([perceptual_features, gripper_feature], dim=2)  #b, 1, 1369*2, 1024
