...
from policy_models.module.depth_model import Depth_UNet

import sys
sys.path.append('/home/sizhe/clear_code/video-prediction-policy/external_toolkits/vggt')
from external_toolkits.vggt.vggt.models.vggt import VGGT
...
class VPP_Policy_with_vggt(pl.LightningModule):
    """
    ...
    def __init__(
            ...
            use_static_vggt = True,
            use_gripper_vggt = False,
            vggt_tokens = 112,
            vggt_former_depth = 3,
        ...
        super(VPP_Policy_with_vggt, self).__init__()

        """
        vggt
        """
        self.use_static_vggt = use_static_vggt
        self.use_gripper_vggt = use_gripper_vggt

        if use_static_vggt or use_gripper_vggt:
            #self.vggt_encoder = VGGT.from_pretrained("/home/sizhe/clear_code/video-prediction-policy/external_toolkits/vggt/official_checkpoints/VGGT-1B").to(self.device)
            vggt = VGGT.from_pretrained("/home/sizhe/clear_code/video-prediction-policy/external_toolkits/vggt/official_checkpoints/VGGT-1B")      # CPU/GPU 都可，稍后再 to(device)
            agg = copy.deepcopy(vggt.aggregator)   
            del vggt                               
            torch.cuda.empty_cache()               
            self.vggt_encoder = agg.to(self.device)
            for p in self.vggt_encoder.parameters():
                p.requires_grad = False
            # 最后一层，64,1,1369,2048
        
        if use_static_vggt:
            self.vggt_video_former_static = Video_Former_3D(
                    dim=latent_dim,
                    depth=vggt_former_depth,
                    dim_head=Former_dim_head,
                    heads=Former_heads,
                    num_time_embeds=1,
                    num_latents=vggt_tokens,
                    condition_dim=2048,
                    use_temporal=False,
                    num_frame=1,
                )
        if use_gripper_vggt:
            self.vggt_video_former_gripper = Video_Former_3D(
                    dim=latent_dim,
                    depth=vggt_former_depth,
                    dim_head=Former_dim_head,
                    heads=Former_heads,
        ...
        if self.pred_depth:
            self.depth_unet = Depth_UNet(depth_unet_cfg)
    ...
    def configure_optimizers(self):
        ...
        modules = [
            "Video_Former",
            "vggt_video_former_static",
            "vggt_video_former_gripper",
            "depth_unet"
        ]
        param_lists = []
        for name in modules:
            if hasattr(self, name):
                param_lists.append(getattr(self, name).parameters())
    

        optim_groups = [
            {"params": self.model.inner_model.parameters(),
             "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": chain(*param_lists), 
             "weight_decay": self.optimizer_config.transformer_weight_decay},
        ]
    ...
    def training_step(self, dataset_batch: Dict[str, Dict],) -> torch.Tensor:  # type: ignore
        ...
        if torch.isnan(dataset_batch["actions"]).any().item():
            print("actions is nan")
        if torch.isnan(dataset_batch["rgb_obs"]["rgb_static"]).any().item():
            print("rgb_static is nan")
        
        predictive_feature, latent_goal= self.extract_predictive_feature(dataset_batch)


        if self.pred_depth:
            depth_condition = predictive_feature['state_images']
            depth_target = dataset_batch["depth_vae"]
            depth_input = dataset_batch["rgb_vae"]  # b, t, 4, 32, 32
            #print("rgb_vae", depth_input.shape)
            #print("depth_target", depth_target.shape)
            depth_input = depth_input.repeat(1, depth_target.shape[1], 1, 1, 1)
            #print("rgb_vae2", depth_input.shape)
            #aaa
            depth_pred = self.depth_unet(depth_input, depth_condition, return_dict = False)
            ...
            depth_loss += (depth_target - depth_pred).abs().mean() * 0.2
            # print("depth_loss:", depth_loss)


        if torch.isnan(predictive_feature["state_images"]).any().item():
            print("predictive_feature[state_images] is nan")
        if torch.isnan(latent_goal).any().item():
            print("latent_goal is nan")
        ...
        if self.pred_depth:
            return total_loss, action_loss, depth_loss
    ...
    def extract_predictive_feature(self, dataset_batch):
        ...
        VGGT spatial branch
        """
        if self.use_static_vggt:
            # print("-1 dataset_batch[rgb_obs][rgb_static]", dataset_batch["rgb_obs"]["rgb_static"].shape)
            rgb_obs_static = (dataset_batch["rgb_obs"]["rgb_static"].squeeze(1) + 1) / 2.0                     # 64, 1, 3, 256, 256
            # print("0 rgb_obs_static", rgb_obs_static.shape)
            rgb_obs_static = torch.nn.functional.interpolate(rgb_obs_static, size=(518, 518)).clamp(0,1).unsqueeze(1)   # 64, 1, 3, 518, 518
            # print("1 rgb_obs_static", rgb_obs_static.shape)

            # print(rgb_obs_static.device)

            rgb_static_tokens_list, patch_start_idx = self.vggt_encoder(rgb_obs_static)
            rgb_static_tokens = rgb_static_tokens_list[-1]  #[:,:,patch_start_idx:]     # 64, 1, 1374, 2048
            # print("2 rgb_obs_static", rgb_static_tokens.shape)

            if torch.isnan(rgb_static_tokens).any().item():
                print("rgb_static_tokens is nan")



            rgb_static_tokens = self.vggt_video_former_static(rgb_static_tokens)

            if torch.isnan(rgb_static_tokens).any().item():
                print("rgb_static_tokens_after_vggt_video_former_static is nan")


            # print("3 rgb_static_tokens:", rgb_static_tokens.shape)

        if self.use_gripper_vggt:
            rgb_obs_gripper = (dataset_batch["rgb_obs"]["rgb_gripper"].squeeze(1) + 1) / 2.0                     # 64, 3, 256, 256
            rgb_obs_gripper = torch.nn.functional.interpolate(rgb_obs_gripper, size=(518, 518)).clamp(0,1).unsqueeze(1)   # 64, 1, 3, 518, 518
            rgb_gripper_tokens_list, patch_start_idx = self.vggt_encoder(rgb_obs_gripper)
            rgb_gripper_tokens = rgb_gripper_tokens_list[-1]  #[:,:,patch_start_idx:]     # 64, 1, 1374, 2048
            rgb_gripper_tokens = self.vggt_video_former_gripper(rgb_gripper_tokens)
        ...
        with torch.no_grad():
            ...
            if torch.isnan(input_rgb).any().item():
                print("input_rgb is nan")
            ...
            if torch.isnan(perceptual_features).any().item():
                print("perceptual_features_TVP_encoder is nan")
        ...
        if self.use_static_vggt:
            # print("before:", perceptual_features.shape, rgb_static_tokens.shape)
            perceptual_features = torch.cat([perceptual_features, rgb_static_tokens], dim=-2)
            # print("after:", perceptual_features.shape)
        if self.use_gripper_vggt:
            # print("before:", perceptual_features.shape, rgb_gripper_tokens.shape)
            perceptual_features = torch.cat([perceptual_features, rgb_gripper_tokens], dim=-2)
            # print("after:", perceptual_features.shape)
    ...
    def eval_forward(self, obs, goal):
        ...
        """
        if self.use_static_vggt:
            # print("-1 dataset_batch[rgb_obs][rgb_static]", obs["rgb_obs"]['rgb_static'].shape)
            rgb_obs_static = (obs["rgb_obs"]['rgb_static'].squeeze(1) + 1) / 2.0                     # 64, 1, 3, 256, 256
            # print("0 rgb_obs_static", rgb_obs_static.shape)
            rgb_obs_static = torch.nn.functional.interpolate(rgb_obs_static, size=(518, 518)).clamp(0,1).unsqueeze(1)   # 64, 1, 3, 518, 518
            # print("1 rgb_obs_static", rgb_obs_static.shape)
    ...
    def inference_depth(self, dataset_batch):
        """
        Method for inference depth video.
        """
        predictive_feature, latent_goal= self.extract_predictive_feature(dataset_batch)
        
        depth_condition = predictive_feature["state_images"]
        depth_input = dataset_batch["rgb_vae"]                 # b, t, 4, 32, 32
        if len(depth_input.shape) == 4:
            depth_input = depth_input.unsqueeze(1)
        depth_input = depth_input.repeat(1, 10, 1, 1, 1)
        #aaa
        depth_pred = self.depth_unet(depth_input, depth_condition, return_dict = False)
        depth_pred = depth_pred[0]
