...
from policy_models.module.Video_Former import Video_Former_2D, Video_Former_3D #, Video_Former_3D_MoE
...
class VPP_Skeleton_vggt_bridge(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        super(VPP_Skeleton_vggt_bridge, self).__init__()
        ...
        elif use_Former=='3d_moe':
            # print("moe_config!", moe_config)
            self.Video_Former = Video_Former_3D_MoE(
                dim=latent_dim,
                depth=Former_depth,
                dim_head=Former_dim_head,
                heads=Former_heads,
        ...
        self.model = GCDenoiser(action_dim = action_dim,
                                ...
                                use_Dual_DiffusionTransformer_Dual_Encoder = True if use_static_vggt else False 
    ...
    def extract_predictive_feature(self, dataset_batch):
        ...
        if self.use_Former=='3d_moe':
            perceptual_features = self.Video_Former(perceptual_features, moe_cond = latent_goal)        #特征编码
        else:
        ...
        if self.use_static_vggt:
            predictive_feature['vggt_features'] = rgb_static_tokens
    ...
    def eval_forward(self, obs, goal):
        ...
        if self.use_static_vggt:
            # print("-1 dataset_batch[rgb_obs][rgb_static]", dataset_batch["rgb_obs"]["rgb_static"].shape)
            rgb_obs_static = (rgb_static.squeeze(1) + 1) / 2.0                    
            # print("0 rgb_obs_static", rgb_obs_static.shape)
            rgb_obs_static = torch.nn.functional.interpolate(rgb_obs_static, size=(518, 518)).clamp(0,1).unsqueeze(1)   # 64, 1, 3, 518, 518
            # print("1 rgb_obs_static", rgb_obs_static.shape)
            ...
            perceptual_emb['vggt_features'] = rgb_static_tokens
    ...
    def step(self, obs, goal, return_chunk = False):
        """
