...
@torch.no_grad()
def basic_ddim_inversion(
    model,
    state,
    x0,                # final denoised result (tensor) -- corresponds to sigmas[-1]
    goal,
    sigmas,            # 1D tensor/list from large -> small (sigmas[0] large, sigmas[-1] ~0)
    scaler=None,
    extra_args=None,
    callback=None,
    disable=None,
    ...
    sigmas_safe = sigmas.clone().to(device=x0.device, dtype=x0.dtype).clamp_min(min_sigma_eps)

    action_next = x0.clone()

    
    sigmas_safe = sigmas_safe[:step+1]
    for i in trange(len(sigmas_safe) - 2, -1, -1, disable=disable):
        sigma_i = sigmas_safe[i]
        sigma_next = sigmas_safe[i + 1]

        A = (sigma_next / sigma_i)   
        C = 1.0 - A

        denoised_hat = model(state, action_next, goal, sigma_next * s_in, **extra_args)

        action_i = (action_next + C * denoised_hat) / A
        ...
        #         'A': A,
        #         'C': C,
        #         'denoised_hat_norm': float(denoised_hat.detach().reshape(denoised_hat.shape[0], -1).norm(dim=1).mean().cpu()),
        #         'action_i_norm': float(action_i.detach().reshape(action_i.shape[0], -1).norm(dim=1).mean().cpu())
        #     })

        action_next = action_i
