...
def process_state(
    ...
    state_obs_keys = observation_space["state_obs"] #只有['robot_obs']
...
def process_rgb(
    ...
    for _, rgb_obs_key in enumerate(rgb_obs_keys):
        ...
        # print("rgb_obs_2", np.min(rgb_obs), np.max(rgb_obs))
        # expand dims for single environment obs
        ...
        # print("rgb_obs_3", torch.min(seq_rgb_obs_), torch.max(seq_rgb_obs_))
...
def process_actions(
    ...
    observation_space: DictConfig,
    transforms: Dict,
    seq_idx: int = 0,               # 实际上就是0
    window_size: int = 0,           # 实际上也是0
) -> Dict[str, torch.Tensor]:
    # shape: (N_actions)
    ...
    if window_size == 0 and seq_idx == 0:  # single file loader         # 走这条路
