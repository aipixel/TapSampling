...
from policy_models.utils.utils import add_text, add_text_up
...
class RolloutVideo:
    ...
    def update(self, rgb_obs: torch.Tensor) -> None:
        ...
        if isinstance(rgb_obs, torch.Tensor):
            ...
            img = torch.from_numpy(rgb_obs).permute(2,0,1)    # c,h,w
            img = F_resize(img, size=(256,256),
        ...
        self.videos[-1] = torch.cat([self.videos[-1], img], dim=1)  # shape 1, t, c, h, w
    ...
    def add_minmax(self, instruction, color=(0,0,0)):
        img_text = np.zeros(self.videos[-1].shape[2:][::-1], dtype=np.uint8) + 127
        add_text_up(img_text, instruction, color)
        img_text = ((img_text.transpose(2, 0, 1).astype(float) / 255.0) * 2) - 1
        self.videos[-1][:, -1 :, ...] += torch.from_numpy(img_text)
        self.videos[-1] = torch.clip(self.videos[-1], -1, 1)
