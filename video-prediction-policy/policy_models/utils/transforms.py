...
class ScaleDepthTensor(object):
    """Scale tensor of shape (batch, C, H, W) containing depth
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        assert isinstance(tensor, torch.Tensor)
        # print("depth_mean_std",torch.mean((tensor.float() - 3.6).div(2.7)), torch.std((tensor.float() - 3.6).div(2.7)))
        return (tensor.float() - 3.6).div(2.7)
        # return tensor.float().div(255)
